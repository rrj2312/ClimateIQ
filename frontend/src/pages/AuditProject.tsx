  ...
  return (
    <div className="container mx-auto flex max-w-2xl flex-col items-center px-6 py-16">
      ...
      <textarea
        ...
        className="mb-4 h-56 w-full resize-none rounded-xl border bg-card p-4 font-mono text-sm text-foreground shadow-blue transition-shadow duration-200 focus:shadow-blue-hover focus:outline-none focus:ring-2 focus:ring-primary/30"
      />

      <div className="mb-6 flex w-full gap-3">
        <button
          onClick={() => setText(SAMPLE_TEXT)}
          className="rounded-xl border px-5 py-2.5 text-sm font-medium text-muted-foreground transition-colors duration-200 hover:text-foreground"
        >
          Load Sample
        </button>
        <button
      ...
      {loading && (
        <div className="w-full rounded-xl bg-card p-8 text-center shadow-blue">
          <Loader2 className="mx-auto mb-3 h-6 w-6 animate-spin text-primary" />
          <p className="text-sm font-medium text-muted-foreground">
            Analyzing claims across 5 dimensions…
          </p>
        </div>
      )}
